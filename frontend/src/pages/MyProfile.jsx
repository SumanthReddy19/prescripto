...
const MyProfile = () => {
  ...
  return userData && (
    <div className="max-w-lg flex flex-col gap-2 text-sm">
      ...
      <div className="mt-10">
        {
          isEdit ? <button className="border border-primary py-2 px-8 rounded-full hover:bg-primary hover:text-white transition-all duration-300" onClick={()=>updateUserProfileData()}>Save Information</button> : <button className="border border-primary py-2 px-8 rounded-full hover:bg-primary hover:text-white transition-all duration-300" onClick={()=>setIsEdit(true)}>Edit</button>
        }
      </div>
    </div>
  );
